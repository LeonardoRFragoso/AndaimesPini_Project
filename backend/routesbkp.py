...
def get_locacoes():
    """
    Rota para listar todas as locações (pedidos) com detalhes do cliente e dos itens locados.
    """
    try:
        locacoes = Locacao.get_all()  # Essa função deve retornar todos os detalhes das locações
        return jsonify(locacoes), 200
    except psycopg2.Error as e:
        print(f"Erro ao buscar locações: {e}")
        return jsonify({"error": "Erro ao buscar locações."}), 500
    except Exception as ex:
    ...
    try:
        ...
        for item in itens_locados:
            ...
            if quantidade is None:
                return jsonify({"error": "Quantidade é obrigatória para todos os itens!"}), 400
...
# Novas rotas para gerenciar a devolução de itens
@routes.route('/locacoes/<int:locacao_id>/devolucao', methods=['POST'])
def devolver_locacao(locacao_id):
    try:
        ...
        ItensLocados.return_item(locacao_id, item_id)  # Exemplo de como poderia ser
        restaurar_estoque(item_id, 1)  # Supondo que estamos devolvendo 1 unidade do item
        return jsonify({"message": "Devolução registrada com sucesso!"}), 200
    except psycopg2.Error as e:
        print(f"Erro ao registrar devolução no banco de dados: {e}")
        return jsonify({"error": "Erro ao registrar devolução."}), 500
    except Exception as ex:
...
def prorrogar_locacao(locacao_id):
    try:
        ...
        if dias_adicionais is None:
            return jsonify({"error": "Dias adicionais são obrigatórios!"}), 400

        # Lógica para prorrogar a locação
        Locacao.extend(locacao_id, dias_adicionais)  # Implementação a ser feita
        return jsonify({"message": "Locação prorrogada com sucesso!"}), 200
    except psycopg2.Error as e:
        print(f"Erro ao prorrogar locação no banco de dados: {e}")
        return jsonify({"error": "Erro ao prorrogar locação."}), 500
    except Exception as ex:
        print(f"Erro inesperado ao prorrogar locação: {ex}")
