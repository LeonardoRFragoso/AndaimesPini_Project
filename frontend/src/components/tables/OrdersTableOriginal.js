...
const OrdersTable = ({ orders, onAction, loadOrders }) => {
  ...
  const handleExtendOrder = async (orderId) => {
    ...
    try {
      const { data } = await axios.put(
        `${API_BASE_URL}/locacoes/${orderId}/prorrogar`,
        {
  ...
  const handleReactivateOrder = async (orderId) => {
    try {
      const response = await axios.patch(
        `${API_BASE_URL}/locacoes/${orderId}/reativar`
      );
      if (response.status === 200) {
        ...
        setSnackbarMessage(
          "Erro ao reativar o pedido. Verifique o status do pedido."
        );
      }
    } catch (error) {
        ...
        <Table>
          <TableHead sx={{ backgroundColor: "#e0f2f1" }}>
            <TableRow>
              ...
              <TableCell
                onClick={() => handleSort("valor_ajustado")}
                style={{ cursor: "pointer" }}
              >
                Valor Ajustado{" "}
                {orderBy.field === "valor_ajustado" &&
                  (orderBy.direction === "asc" ? (
                    <ArrowUpward fontSize="small" />
                  ) : (
                    <ArrowDownward fontSize="small" />
          ...
          <TableBody>
            {sortedOrders.map((order) => {
              const valorAjustado = order.novo_valor_total
                ...
                : order.valor_receber_final ?? order.valor_total ?? 0;
              return (
                <TableRow
                  key={order.id}
                  sx={{
                ...
                >
                  ...
                  <TableCell>{`R$ ${valorAjustado.toFixed(2)}`}</TableCell>
                  <TableCell>
                    <Chip
                      label={
                        order.status === "concluido" ? "Concluído" : "Ativo"
                      }
                      color={order.status === "concluido" ? "success" : "error"}
                      sx={{
                        backgroundColor:
                          order.status === "concluido" ? "#d1f7d1" : "#ffebee",
                        color:
                          order.status === "concluido" ? "#388e3c" : "#d32f2f",
                        fontWeight: "bold",
                      }}
                      icon={
        ...
        <DialogContent dividers>
          {selectedOrder && (
            <>
              ...
              <Typography>
                Valor Total: R$ {(selectedOrder.valor_total || 0).toFixed(2)}
              </Typography>
              <Typography>
                Valor Pago na Entrega: R${" "}
                {(selectedOrder.valor_pago_entrega || 0).toFixed(2)}
              </Typography>
              <Typography>
                Valor a Receber no Final: R${" "}
                {(selectedOrder.valor_receber_final || 0).toFixed(2)}
              </Typography>
              <Typography>
                Valor Abatimento: R${" "}
                {(selectedOrder.abatimento || 0).toFixed(2)}
              </Typography>
              <Typography>
                Novo Valor Total: R${" "}
                {(selectedOrder.novo_valor_total || 0).toFixed(2)}
              </Typography>
              {
                // Exibir "Valor Ajustado Final" somente se novo_valor_total ou abatimento forem maiores que zero
                (selectedOrder.novo_valor_total > 0 ||
                  selectedOrder.abatimento > 0) && (
                  <Typography>
